...
class ClarityGraphPlugin extends siyuan.Plugin {
  constructor() {
    ...
    __publicField(this, "cachedBounds");
    __publicField(this, "cachedStageDims", { width: 900, height: 620 });
    __publicField(this, "cachedCanvasRect", { left: 0, top: 0 });
    __publicField(this, "cachedVisibleNodes", []);
    __publicField(this, "cachedVisibleLinks", []);
    __publicField(this, "hoveredNodeId");
  ...
  async collectNotebookDocs(notebook, path, ancestors, parentId) {
  ...
  startAnimatedSim() {
    ...
    this.simState = {
      nodes,
      links: this.graph.links.filter((l) => visible.has(l.source) && visible.has(l.target)).map((l) => ({ source: byId.get(l.source), target: byId.get(l.target), count: l.count })),
  ...
  stopSimFrame() {
    if (this.simFrame === void 0) return;
    window.cancelAnimationFrame(this.simFrame);
    this.simFrame = void 0;
    this.simState = void 0;
  }
  visibleNodes() {
  ...
  draw() {
    ...
    const canvas = this.root.querySelector(".cg-canvas");
    ...
    this.cachedVisibleLinks = this.graph.links.filter((link) => visible.has(link.source) && visible.has(link.target)).map((link) => ({ source: byId.get(link.source), target: byId.get(link.target), count: link.count })).filter((link) => link.source && link.target);
  ...
  startPulseLoop() {
    if (this.pulseFrame !== void 0) return;
    const loop = () => {
      this.paintCanvas();
  ...
  updateHoverFromPointer(event) {
    ...
    if (!node) {
      ...
      this.hideTooltip();
      if (prevId !== void 0) this.scheduleViewTransform();
      return;
    }
    if (node.id !== prevId) {
...
function flattenInlineDocs(files, notebook, ancestors, parentId) {
  const docs = [];
  for (const file of files) {
    if (!file.id) continue;
    const title = cleanDocTitle(String(file.title || file.name || "Untitled"));
